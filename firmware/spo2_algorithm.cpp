...
void maxim_heart_rate_and_oxygen_saturation(uint32_t *pun_ir_buffer, int32_t n_ir_buffer_length, uint32_t *pun_red_buffer, int32_t *pn_spo2, int8_t *pch_spo2_valid, int32_t *pn_heart_rate, int8_t *pch_hr_valid) {
  ...
  int32_t an_ratio[5], an_x[5], an_y[5];
  ...
  if (n_npks >= 2) {
    ...
    *pn_heart_rate = (int32_t)((float)FS * 60 / n_peak_interval_sum);
    *pch_hr_valid = 1;
  } else {
  ...
  for (k = 0; k < n_exact_ir_valley_locs_count - 1; k++) {
    ...
    if (an_ir_valley_locs[k + 1] - an_ir_valley_locs[k] > 3) {
      for (i = an_ir_valley_locs[k]; i < an_ir_valley_locs[k + 1]; i++) {
        if (pun_ir_buffer[i] > n_x_dc_max) {
          n_x_dc_max = pun_ir_buffer[i];
          n_x_dc_max_idx = i;
        }
        if (pun_red_buffer[i] > n_y_dc_max) {
          n_y_dc_max = pun_red_buffer[i];
          n_y_dc_max_idx = i;
        }
      }
      ...
      n_y_ac = n_y_dc_max - n_y_ac; // subracting linear DC compoenent from raw
      ...
      n_x_ac = n_x_dc_max - n_x_ac; // subracting linear DC compoenent from raw
  ...
  for (i = -1; i < *pn_npks; i++) {
    ...
    *pn_npks = i + 1;
    for (j = i + 1; j < n_old_npks; j++) {
      n_dist = pn_locs[j] - (i == -1 ? -1000 : pn_locs[i]); // lag-zero peak of autocorr is at index -1
...
// Missing constants that should be defined
#define FS 25 // Sampling frequency
static int32_t n_nume, n_denom;